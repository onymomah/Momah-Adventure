...
function BattleArenaMode({ provider, apiKey, muted, setMuted, childMode, onBack }) {
  ...
  const M = "'Courier New',monospace", S = "'Georgia','Times New Roman',serif";
  ...
  const page = { background: '#080705', minHeight: '100vh', fontFamily: S, color: '#ddd6c8', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' };
  const crd = (on) => ({ background: on ? ADim : '#0c0a08', border: `1px solid ${on ? AC : '#1c1916'}`, borderRadius: '8px', padding: '20px', cursor: 'pointer', transition: 'all 0.15s' });
  const chip = (on, cc) => ({ background: on ? (cc || AC) : 'transparent', border: `1px solid ${on ? (cc || AC) : '#221e1a'}`, borderRadius: '20px', padding: '5px 12px', cursor: 'pointer', fontFamily: M, fontSize: '9px', letterSpacing: '1px', color: on ? '#080705' : '#4a4540', transition: 'all 0.15s' });
  const inp = { background: '#0c0a08', border: '1px solid #221e1a', borderRadius: '6px', padding: '10px 13px', color: '#ddd6c8', fontSize: '14px', width: '100%', outline: 'none', fontFamily: S, boxSizing: 'border-box' };
  const Ey = ({ c }) => <div style={{ fontFamily: M, fontSize: '7px', letterSpacing: '5px', color: '#252118', marginBottom: '8px' }}>{c}</div>;
  const H1 = ({ c }) => <h1 style={{ fontSize: '30px', fontWeight: '700', margin: '0 0 6px', letterSpacing: '-0.5px', fontFamily: S, lineHeight: 1.1 }}>{c}</h1>;
  const Sub = ({ c }) => <p style={{ color: '#3d3830', fontFamily: M, fontSize: '8px', letterSpacing: '2px', margin: '0 0 28px' }}>{c}</p>;
  const Lbl = ({ c }) => <div style={{ fontFamily: M, fontSize: '7px', letterSpacing: '3px', color: '#2d2924', marginBottom: '6px' }}>{c}</div>;
  const PB = ({ on, onClick, sx = {}, children }) => <button onClick={onClick} style={{ background: on ? AC : '#181410', border: `1px solid ${on ? AC : '#221e1a'}`, borderRadius: '6px', padding: '11px 28px', color: on ? '#080705' : '#302a25', cursor: on ? 'pointer' : 'not-allowed', fontFamily: M, fontSize: '9px', letterSpacing: '3px', fontWeight: '700', transition: 'all 0.15s', ...sx }}>{children}</button>;
  const BK = ({ onClick }) => <button onClick={onClick} style={{ background: 'transparent', border: '1px solid #181410', borderRadius: '6px', padding: '10px 18px', color: '#302a25', cursor: 'pointer', fontFamily: M, fontSize: '8px', letterSpacing: '2px' }}>BACK</button>;
  const Narr = ({ text, dim }) => { if (!text) return null; return <div style={{ lineHeight: 1.95, fontSize: '16px', color: dim ? '#252118' : '#b8b0a4', fontFamily: S }}>{text.split('\n').map((ln, i) => ln.trim() === '' ? <div key={i} style={{ height: '8px' }} /> : <p key={i} style={{ margin: '0 0 4px' }}>{ln}</p>)}</div>; };
  ...
  // ── Forge Modal ──
  const ForgeModal = () => (
    <div style={{ position: 'fixed', inset: 0, background: '#080705ee', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
      <div style={{ background: '#0c0a08', border: `1px solid ${ABorder}`, borderRadius: '10px', padding: '26px', maxWidth: '500px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
          <div style={{ fontSize: '18px', fontWeight: '700', fontFamily: S }}>Character Forge</div>
          <button onClick={() => setForgeOpen(false)} style={{ background: 'transparent', border: '1px solid #1c1916', borderRadius: '5px', padding: '4px 10px', color: '#3d3830', cursor: 'pointer', fontFamily: M, fontSize: '8px' }}>CLOSE</button>
        </div>
        {!forgeResult ? (
          <>
            <Lbl c="CHOOSE ARCHETYPE" />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '9px', marginBottom: '16px' }}>
              {ARCHETYPES.map(g => <div key={g.group}><div style={{ fontFamily: M, fontSize: '6px', color: '#252118', letterSpacing: '2px', marginBottom: '5px' }}>{g.group.toUpperCase()}</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>{g.options.map(o => <div key={o} onClick={() => setForgeArch(o)} style={chip(forgeArch === o)}>{o}</div>)}</div></div>)}
            </div>
            <Lbl c="OPTIONAL DETAILS" />
            <textarea value={forgeDetails} onChange={e => setForgeDetails(e.target.value)} placeholder="e.g. Ancient warrior, honor-bound" style={{ ...inp, height: '60px', resize: 'none', marginBottom: '14px' }} />
            {forgeError && <div style={{ color: '#d06060', fontFamily: M, fontSize: '9px', marginBottom: '10px' }}>{forgeError}</div>}
            <PB on={!!forgeArch && !forgeLoading} onClick={runForge} sx={{ width: '100%' }}>{forgeLoading ? `FORGING${'.'.repeat(dots)}` : 'FORGE CHARACTER'}</PB>
          </>
        ) : (
          <>
            <div style={{ background: '#0a0806', border: '1px solid #1c1916', borderRadius: '8px', padding: '16px', marginBottom: '14px' }}>
              <div style={{ fontSize: '20px', fontWeight: '700', fontFamily: S, marginBottom: '3px' }}>{forgeResult.name}</div>
              <div style={{ fontFamily: M, fontSize: '8px', color: AC, letterSpacing: '2px', marginBottom: '12px' }}>{forgeResult.title}</div>
              {[['APPEARANCE', forgeResult.appearance], ['COMBAT STYLE', forgeResult.combatStyle], ['CENTRAL DOGMA', forgeResult.centralDogma], ['ABILITIES', forgeResult.abilities], ['FLAW', forgeResult.flaw]].map(([lb, val]) => val ? <div key={lb} style={{ marginBottom: '9px' }}><Lbl c={lb} /><div style={{ fontSize: '13px', color: '#8a8278', lineHeight: 1.7, fontFamily: S }}>{val}</div></div> : null)}
              {forgeResult.voiceSample && <div style={{ borderTop: '1px solid #1c1916', paddingTop: '8px', marginTop: '5px', fontSize: '13px', color: '#5a5248', fontStyle: 'italic', fontFamily: S }}>"{forgeResult.voiceSample}"</div>}
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setForgeResult(null)} style={{ flex: 1, background: 'transparent', border: '1px solid #1c1916', borderRadius: '6px', padding: '10px', color: '#3d3830', cursor: 'pointer', fontFamily: M, fontSize: '7px', letterSpacing: '2px' }}>REFORGE</button>
    ...
    <div style={page}>
      ...
      <div style={{ textAlign: 'center', maxWidth: '580px', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          ...
          <button onClick={onBack} style={{ background: 'transparent', border: '1px solid #1c1916', borderRadius: '5px', padding: '4px 10px', color: '#3d3830', cursor: 'pointer', fontFamily: M, fontSize: '7px', letterSpacing: '2px' }}>← MENU</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '28px' }}>
          {[{ id: 'kids_young', c: '#22c55e', lbl: 'KIDS JUNIOR', sub: 'AGES 6-8', desc: 'Fun and silly. BOOM! CRASH! Simple words. Win through heart and cleverness.' }, { id: 'kids_older', c: '#f59e0b', lbl: 'KIDS', sub: 'AGES 9-12', desc: 'Cinematic action. Real strategy. Vivid consequences. Read-aloud ready.' }, ...(!childMode ? [{ id: 'intense', c: '#4f8ef7', lbl: 'INTENSE', sub: 'AGES 13+', desc: 'High stakes. Gritty damage. Tactics decide everything. TTS-optimized.' }, { id: 'brutal', c: '#e03c3c', lbl: 'BRUTAL', sub: 'ADULT', desc: 'Forensic. Detached. Terminal Report. Psychological dissolution.' }] : [])].map(t => (
            <div key={t.id} onClick={() => setTone(t.id)} style={{ background: tone === t.id ? `${t.c}12` : '#0c0a08', border: `1px solid ${tone === t.id ? t.c : '#1c1916'}`, borderRadius: '10px', padding: '18px', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'left' }}>
              <div style={{ fontFamily: M, fontSize: '11px', fontWeight: '700', color: tone === t.id ? t.c : '#5a5248', marginBottom: '3px', letterSpacing: '2px' }}>{t.lbl}</div>
              <div style={{ fontFamily: M, fontSize: '6px', color: '#252118', letterSpacing: '2px', marginBottom: '9px' }}>{t.sub}</div>
              <div style={{ fontSize: '12px', color: '#3d3830', lineHeight: 1.65, fontFamily: S }}>{t.desc}</div>
            </div>
          ))}
        </div>
    ...
    <div style={page}>
      <div style={{ textAlign: 'center', maxWidth: '560px', width: '100%' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
          {[{ id: '1v1', lbl: '1 vs 1', icon: '⚔', desc: 'Pure duel.' }, { id: 'team', lbl: 'Teams', icon: '🛡', desc: '2-4 fighters per side.' }, { id: 'ffa', lbl: 'Free For All', icon: '💥', desc: '3-8 fighters.' }, { id: 'tournament', lbl: 'Tournament', icon: '🏆', desc: '4-8 fighters, bracket.' }].map(t => (
            <div key={t.id} onClick={() => setBattleType(t.id)} style={crd(battleType === t.id)}>
              <div style={{ fontSize: '22px', marginBottom: '6px' }}>{t.icon}</div>
              <div style={{ fontFamily: M, fontSize: '11px', fontWeight: '700', color: battleType === t.id ? AC : '#5a5248', marginBottom: '7px', letterSpacing: '1px' }}>{t.lbl}</div>
              <div style={{ fontSize: '12px', color: '#3d3830', lineHeight: 1.65, fontFamily: S }}>{t.desc}</div>
            </div>
          ))}
        </div>
        {battleType === 'team' && <div style={{ background: '#0c0a08', border: '1px solid #181410', borderRadius: '7px', padding: '14px', marginBottom: '16px' }}><Lbl c="FIGHTERS PER SIDE" /><div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>{[2, 3, 4].map(n => <div key={n} onClick={() => setTeamSize(n)} style={{ background: teamSize === n ? ADim : 'transparent', border: `1px solid ${teamSize === n ? AC : '#221e1a'}`, borderRadius: '6px', padding: '8px 20px', cursor: 'pointer', fontFamily: M, fontSize: '11px', color: teamSize === n ? AC : '#4a4540' }}>{n}</div>)}</div></div>}
    ...
    return (
      <div style={page}>
        ...
        <div style={{ textAlign: 'center', maxWidth: '540px', width: '100%' }}>
          ...
          {(isFFA || isTourney) && (
            <div style={{ textAlign: 'left', marginBottom: '16px' }}>
              {slots.map((f, i) => <div key={i} style={{ marginBottom: '8px', display: 'flex', gap: '7px', alignItems: 'center' }}><div style={{ fontFamily: M, fontSize: '8px', color: f.trim() ? AC : '#252118', width: '18px', flexShrink: 0 }}>{i + 1}.</div><input value={f} onChange={e => { const n = [...slots]; n[i] = e.target.value; setSlots(n); }} placeholder={`Fighter ${i + 1}${i < minSlots ? ' (required)' : ''}`} style={{ ...inp, borderColor: f.trim() ? ABorder : '#221e1a' }} />{!isKids && <button onClick={() => openForge(i)} style={{ background: 'transparent', border: `1px solid ${ABorder}`, borderRadius: '4px', padding: '6px 9px', color: AC, cursor: 'pointer', fontFamily: M, fontSize: '7px', flexShrink: 0 }}>⚡</button>}</div>)}
            </div>
          )}
          {showWC && (
        ...
        <div style={{ textAlign: 'center', maxWidth: '520px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', alignItems: 'center' }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: '#4a4540', pointerEvents: 'none', fontSize: '11px' }}>▼</div>
            </div>
          ...
          {arena && arena !== 'custom' && <div style={{ background: ADim, border: `1px solid ${ABorder}`, borderRadius: '6px', padding: '10px 14px', marginBottom: '12px', textAlign: 'left' }}><div style={{ fontFamily: M, fontSize: '7px', color: AC, letterSpacing: '3px', marginBottom: '3px' }}>SELECTED</div><div style={{ fontSize: '15px', color: '#ddd6c8', fontFamily: S }}>{arena}</div></div>}
      ...
      <div style={page}>
        <div style={{ textAlign: 'center', maxWidth: '460px', width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '20px' }}>
            {OBJECTIVES.map(o => <div key={o} onClick={() => setObjective(o)} style={{ background: objective === o ? ADim : '#0c0a08', border: `1px solid ${objective === o ? AC : '#1c1916'}`, borderRadius: '7px', padding: '12px', cursor: 'pointer', textAlign: 'center', fontFamily: M, fontSize: '8px', letterSpacing: '1px', color: objective === o ? AC : '#3d3830', fontWeight: objective === o ? '700' : '400' }}>{o.toUpperCase()}</div>)}
          </div>
  ...
  if (screen === 'briefing') return (
    <div style={page}>
      <div style={{ textAlign: 'center', maxWidth: '500px', width: '100%' }}>
        ...
        {briefing && (
          <div style={{ background: '#0c0a08', border: `1px solid ${ABorder}`, borderRadius: '10px', padding: '22px', marginBottom: '22px', textAlign: 'left' }}>
            {tone === 'brutal' ? (
  ...
  if (screen === 'story' || screen === 'resolution') {
    ...
    return (
      <div style={{ background: '#080705', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: S, color: '#ddd6c8' }}>
        <div style={{ padding: '9px 18px', borderBottom: '1px solid #111009', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, gap: '10px' }}>
          <div style={{ minWidth: 0 }}><div style={{ fontFamily: M, fontSize: '6px', letterSpacing: '4px', color: '#1c1814', marginBottom: '2px' }}>STORYVERSE BATTLE</div><div style={{ fontSize: '11px', color: '#5a5248', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}><span style={{ fontFamily: M, fontSize: '7px', color: AC, letterSpacing: '1px', marginRight: '6px' }}>{battleType.toUpperCase()}</span><span style={{ color: '#4a4540' }}>{finalArena}</span></div></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
            {latestNarr && !resolution && <button onClick={isSpeaking ? stopSpeaking : () => readAloud(latestNarr)} style={{ background: isSpeaking ? `${AC}20` : 'transparent', border: `1px solid ${isSpeaking ? AC : '#1c1814'}`, borderRadius: '5px', padding: '4px 9px', color: isSpeaking ? AC : '#3d3830', cursor: 'pointer', fontFamily: M, fontSize: '7px' }}>{isSpeaking ? '◼ STOP' : '▶ READ'}</button>}
            <button onClick={toggleAudio} style={{ background: 'transparent', border: `1px solid ${audioOn ? '#2a2520' : '#181410'}`, borderRadius: '5px', padding: '4px 9px', color: audioOn ? '#5a5248' : '#2a2520', cursor: 'pointer', fontFamily: M, fontSize: '7px' }}>{audioOn ? '♪ ON' : '♪ OFF'}</button>
            {curPhase && <span style={{ fontFamily: M, fontSize: '6px', letterSpacing: '3px', color: AC, opacity: 0.6 }}>{PH[curPhase] || curPhase.toUpperCase()}</span>}
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '40px', height: '2px', background: '#1a1714', borderRadius: '2px', overflow: 'hidden' }}><div style={{ width: `${pct}%`, height: '100%', background: AC, transition: 'width 0.6s' }} /></div><span style={{ fontFamily: M, fontSize: '6px', color: '#1c1814' }}>{beatCount}/{maxBeats}</span></div>
            <button onClick={resetBattle} style={{ background: 'transparent', border: '1px solid #181410', borderRadius: '5px', padding: '3px 9px', color: '#2d2924', cursor: 'pointer', fontFamily: M, fontSize: '7px', letterSpacing: '2px' }}>NEW</button>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '26px 18px' }}>
          <div style={{ maxWidth: '640px', margin: '0 auto' }}>
            {beats.map((beat, i) => <div key={i} style={{ marginBottom: '24px' }}><Narr text={beat.narrative} dim={true} />{beat.choiceMade && <div style={{ marginTop: '8px', display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '3px 10px', background: beat.wasWildcard ? `${WCC}10` : `${AC}10`, border: `1px solid ${beat.wasWildcard ? WCC : AC}22`, borderRadius: '4px' }}><span style={{ fontFamily: M, fontSize: '6px', letterSpacing: '2px', color: beat.wasWildcard ? WCC : '#1c1814' }}>{beat.wasWildcard ? '⚡ WILDCARD' : 'CHOSE'}</span><span style={{ fontFamily: M, fontSize: '8px', color: beat.wasWildcard ? WCC : AC }}>{beat.choiceMade}</span></div>}{i < beats.length - 1 && <div style={{ height: '1px', background: '#0f0e0c', margin: '20px 0 0' }} />}</div>)}
            {currentBeat && !resolution && <Narr text={currentBeat.narrative} dim={false} />}
            {loading && <div style={{ padding: '32px 0' }}><div style={{ fontFamily: M, fontSize: '6px', letterSpacing: '4px', color: '#1c1814', marginBottom: '8px' }}>GENERATING</div><div style={{ display: 'flex', gap: '5px' }}>{[1, 2, 3].map(d => <div key={d} style={{ width: '4px', height: '4px', borderRadius: '50%', background: AC, opacity: dots >= d ? 0.85 : 0.12 }} />)}</div></div>}
            {error && <div style={{ padding: '10px 13px', background: '#120808', border: '1px solid #2c1010', borderRadius: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}><span style={{ fontSize: '10px', color: '#d07070', fontFamily: M }}>{error}</span><button onClick={() => setError('')} style={{ background: 'transparent', border: '1px solid #2c1010', borderRadius: '4px', padding: '3px 8px', color: '#b05050', cursor: 'pointer', fontSize: '7px', fontFamily: M }}>DISMISS</button></div>}
            {resolution && (
              <div style={{ marginTop: '28px' }}>
                <div style={{ height: '1px', background: `${AC}40`, marginBottom: '28px' }} />
                <div style={{ padding: '24px', background: '#0c0a08', borderRadius: '10px', border: `1px solid ${ABorder}` }}>
                  {tone === 'brutal' ? (
                    <><div style={{ fontFamily: M, fontSize: '7px', letterSpacing: '4px', color: AC, marginBottom: '18px' }}>TERMINAL REPORT</div><div style={{ marginBottom: '18px' }}><Lbl c="VICTOR" /><div style={{ fontSize: '26px', fontWeight: '700', fontFamily: S }}>{resolution.winner}</div></div>{resolution.definingMoment && <div style={{ marginBottom: '14px' }}><Lbl c="THE DEFINING MOMENT" /><div style={{ fontSize: '14px', color: '#6a6258', lineHeight: 1.85, fontStyle: 'italic', fontFamily: S }}>{resolution.definingMoment}</div></div>}{resolution.indifferentLaw && <div style={{ marginBottom: '14px' }}><Lbl c="THE INDIFFERENT LAW" /><div style={{ fontSize: '14px', color: '#6a6258', lineHeight: 1.85, fontFamily: S }}>{resolution.indifferentLaw}</div></div>}{resolution.scaredInstrument && <div style={{ marginBottom: '14px', background: '#0a0806', borderRadius: '7px', padding: '12px', border: '1px solid #181410' }}><Lbl c="THE SCARRED INSTRUMENT" /><div style={{ fontFamily: M, fontSize: '9px', color: AC, letterSpacing: '1px', marginBottom: '5px' }}>{resolution.scaredInstrument.name}</div><div style={{ fontSize: '13px', color: '#5a5248', lineHeight: 1.8, fontFamily: S, fontStyle: 'italic' }}>{resolution.scaredInstrument.sacrifice}</div></div>}{resolution.hollowLine && <div style={{ borderTop: '1px solid #181410', paddingTop: '14px' }}><div style={{ fontSize: '16px', color: '#4a4540', fontStyle: 'italic', lineHeight: 1.85, fontFamily: S }}>"{resolution.hollowLine}"</div></div>}</>
                  ) : (
                    <><div style={{ fontFamily: M, fontSize: '7px', letterSpacing: '4px', color: AC, marginBottom: '18px' }}>{isKids ? 'BATTLE OVER!' : 'BATTLE COMPLETE'}</div><div style={{ marginBottom: '18px' }}><Lbl c="WINNER" /><div style={{ fontSize: '26px', fontWeight: '700', fontFamily: S }}>{resolution.winner}</div></div>{resolution.winReason && <div style={{ fontSize: '14px', color: '#7a7268', lineHeight: 1.85, marginBottom: '14px', fontFamily: S }}>{resolution.winReason}</div>}{resolution.mvp && <div style={{ background: '#0a0806', borderRadius: '7px', padding: '12px', border: '1px solid #181410', marginBottom: '14px' }}><Lbl c="MVP" /><div style={{ fontFamily: M, fontSize: '9px', color: AC, letterSpacing: '1px', marginBottom: '4px' }}>{resolution.mvp.name}</div><div style={{ fontSize: '13px', color: '#5a5248', lineHeight: 1.8, fontFamily: S }}>{resolution.mvp.keyMoment}</div></div>}{resolution.victoryLine && <div style={{ padding: '11px 15px', background: '#080705', borderRadius: '6px', marginBottom: '14px', fontSize: '15px', color: '#8a8278', fontStyle: 'italic', lineHeight: 1.75, border: '1px solid #181410', fontFamily: S }}>"{resolution.victoryLine}"</div>}{resolution.lesson && <><Lbl c={isKids ? 'THE LESSON' : 'WHAT DECIDED IT'} /><div style={{ fontSize: '13px', color: '#4a4540', lineHeight: 1.85, fontFamily: S }}>{resolution.lesson}</div></>}</>
                  )}
                </div>
        ...
        {currentBeat && !loading && !resolution && currentBeat.choices && (
          <div style={{ borderTop: '1px solid #0f0e0c', background: '#060504', padding: '12px 18px', flexShrink: 0 }}>
            <div style={{ maxWidth: '640px', margin: '0 auto' }}>
              <div style={{ fontFamily: M, fontSize: '6px', letterSpacing: '4px', color: '#1c1814', marginBottom: '7px' }}>YOUR MOVE</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                {currentBeat.choices.map(ch => {
                  const isWC = ch.id === 4;
                  return <button key={ch.id} onClick={() => handleChoice(ch)} style={{ background: '#0c0a08', border: `1px solid ${isWC ? WCBorder : '#1c1916'}`, borderRadius: '6px', padding: '9px 13px', color: '#ddd6c8', cursor: 'pointer', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '3px', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = isWC ? WCC : AC; e.currentTarget.style.background = isWC ? WCDim : ADim; }} onMouseLeave={e => { e.currentTarget.style.borderColor = isWC ? WCBorder : '#1c1916'; e.currentTarget.style.background = '#0c0a08'; }}><div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>{isWC && <span style={{ fontSize: '10px' }}>⚡</span>}<span style={{ fontFamily: M, fontSize: '8px', fontWeight: '700', letterSpacing: '1px', color: isWC ? WCC : '#c8c0b4' }}>{ch.text.toUpperCase()}</span>{isWC && <span style={{ fontFamily: M, fontSize: '6px', color: WCC, letterSpacing: '2px', opacity: 0.7 }}>WILDCARD</span>}</div><span style={{ fontSize: '12px', color: isWC ? '#7a6038' : '#3d3830', fontFamily: S }}>{ch.detail}</span></button>;
                })}
              </div>
            </div>
          </div>
